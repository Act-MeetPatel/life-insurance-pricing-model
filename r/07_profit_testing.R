# ---- Piece 1: Load libraries, assumptions, and Phase 6 pricing functions ----

library(readxl)
library(dplyr)

# Read assumptions from Excel
...
# Mortality lookup: maps any single age to its 5-year band's pricing_qx
...
ggplot(cf, aes(x = policy_year, y = net_cashflow)) +
  geom_col(fill = "steelblue") +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +
  labs(
    title = "Emerging Profit Pattern — Undiscounted Net Cash Flow by Policy Year",
    subtitle = "Age 40, Nonsmoker, 3-Year Term, $100,000 Face Amount",
    x = "Policy Year",
    y = "Net Cash Flow ($)"
  ) +
  theme_minimal()
